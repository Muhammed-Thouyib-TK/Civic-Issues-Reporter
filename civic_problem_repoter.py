...
def admin_dashboard(df, users):
    ...
    if not df.empty:
        ...
        if not df_time.empty:
            ...
            st.info("No timestamp data available to show Reports Over Time.")
    ...
    st.download_button(
        ...
        data=users.to_csv(index=False).encode('utf-8'),
        file_name="users.csv",
        mime="text/csv"
    )
    ...
    # ---------------- ADMIN PAGES ----------------
        ...
        if nav == "Dashboard":
            ...
            if not filtered_df.empty:
                ...
                df_time = filtered_df.dropna(subset=["timestamp"])
                if not df_time.empty:
                    time_group = df_time.groupby(df_time["timestamp"].dt.date).size().reset_index(name="Reports")
                ...
                loc_counts.columns = ["Location", "Reports"]
                fig8 = px.bar(loc_counts, x="Location", y="Reports")
