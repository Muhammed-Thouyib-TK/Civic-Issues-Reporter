...
def main_app():
    ...
    # ===================== USER PAGES =====================
    if user["role"] != "Admin":
        # ... (All user pages remain as previously implemented)
        # Home, Report Issue, My Reports, Notifications, Profile, Contact Support, Feedback
        # Like/Dislike, update reports, prevent duplicates handled

        # For brevity, existing user page code remains unchanged
        ...
        if nav == "Dashboard":
            ...
            if df.empty:
                ...
                for _, row in df.iterrows():
                    ...
                    with st.expander("Update Report Status"):
                        ...
                        update_btn = st.button("Update Status", key=f"update_status_{row['id']}")
                        if update_btn:
                            df.loc[df['id']==row['id'],'status'] = new_status
                            save_data(df)
                            notif = load_notifications()
                            notif = pd.concat([notif,pd.DataFrame([{
                                "user": row['user'],
                                "message": f"Admin changed status of your report '{row['title']}' to {new_status}",
            ...
            if notif.empty:
                ...
                for _, n in notif.iterrows():
                    st.write(f"{n['timestamp']} - {n['user']}: {n['message']}")
            ...
            st.subheader("📄 Feedback & Support Messages")
            ...
            if feedback.empty:
                ...
                st.download_button("📥 Download Feedback CSV", data=feedback.to_csv(index=False).encode('utf-8'), file_name="feedback.csv", mime="text/csv")

            st.markdown("### Support Messages")
            if support.empty:
                st.info("No support messages.")
            else:
                st.dataframe(support)
                st.download_button("📥 Download Support CSV", data=support.to_csv(index=False).encode('utf-8'), file_name="support.csv", mime="text/csv")
