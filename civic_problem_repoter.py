...
def load_artifacts():
    artifacts = {}
    try:
        artifacts["tfidf"] = joblib.load(f"{MODELS_DIR}/tfidf_vectorizer.pkl")
        artifacts["svd"] = joblib.load(f"{MODELS_DIR}/svd_transformer.pkl")
        artifacts["clf"] = joblib.load(f"{MODELS_DIR}/clf.pkl")
        artifacts["label_encoder"] = joblib.load(f"{MODELS_DIR}/label_encoder.pkl")
    except:
        artifacts = None
    return artifacts
...
# Utils
# ----------------------------
def sha256_hash(txt): return hashlib.sha256(txt.encode()).hexdigest()
...
def save_df(df, file): df.to_csv(file, index=False)
...
# ----------------------------
    ...
    with st.form("signup"):
        ...
        if s:
            ...
            if u in df["username"].values: st.error("Username exists")
            else:
                row = {"id":str(uuid.uuid4())[:8],"username":u,"email":e,"pw":sha256_hash(p),"is_admin":False}
...
# ----------------------------
    ...
    with st.form("login"):
        ...
        if s:
            ...
            if user.empty: st.error("User not found")
            else:
                row = user.iloc[0]
    ...
    with st.form("report"):
        ...
        if s:
            ...
            if f:
                ...
                Image.open(f).save(img_path)
            df = load_df(ISSUES_FILE, ["id","user","title","desc","pred","conf","status","created_at","image"])
            row = {"id":str(uuid.uuid4())[:8],"user":st.session_state.user["username"],"title":t,"desc":d,"pred":pred,"conf":conf,"status":"Pending","created_at":datetime.now().isoformat(),"image":img_path}
            df = pd.concat([df,pd.DataFrame([row])], ignore_index=True)
...
# ----------------------------
    ...
    if my.empty: st.info("No reports yet.")
    else: st.dataframe(my[["id","title","pred","status","created_at"]])
...
# ----------------------------
    ...
    for i in imgs:
        if i and os.path.exists(i): st.image(i, width=300)
