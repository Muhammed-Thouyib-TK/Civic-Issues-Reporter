...
try:
    ...
    img_scaler = joblib.load("img_scaler.pkl")
...
def load_data():
    ...
    return pd.DataFrame(columns=["id", "title", "description", "category", "status", "image", "timestamp", "user", "location"])
...
def main_app():
    ...
    if nav == "Home":
            ...
            if submitted:
                if title and description and location:
                    ...
                    if not duplicate.empty:
                        st.warning("⚠️ This issue seems to be already reported by another user. It will still be saved.")
        ...
        if df.empty:
            ...
            users = load_users()
            st.dataframe(users)
            st.download_button("⬇️ Export Users CSV", data=users.to_csv(index=False), file_name="users.csv", mime="text/csv")
